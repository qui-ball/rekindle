...
class TestRestorationJobModel:
    ...
    def test_restoration_job_default_values(self, test_db_session):
        ...
        job = RestorationJob(
            user_id="test_user", original_image_url="https://test.example.com/image.jpg"
        )
    ...
    def test_restoration_job_user_filtering(self, test_db_session):
        ...
        user1_job = RestorationJob(
            user_id="user1", original_image_url="https://test.example.com/user1.jpg"
        )
        user2_job = RestorationJob(
            user_id="user2", original_image_url="https://test.example.com/user2.jpg"
        )
        ...
        # Act
        user1_jobs = (
            test_db_session.query(RestorationJob)
            .filter(RestorationJob.user_id == "user1")
            .all()
        )
        user2_jobs = (
            test_db_session.query(RestorationJob)
            .filter(RestorationJob.user_id == "user2")
            .all()
        )
    ...
    def test_restoration_job_status_filtering(self, test_db_session):
        ...
        pending_jobs = (
            test_db_session.query(RestorationJob)
            .filter(RestorationJob.status == JobStatus.PENDING)
            .all()
        )
        completed_jobs = (
            test_db_session.query(RestorationJob)
            .filter(RestorationJob.status == JobStatus.COMPLETED)
            .all()
        )
    ...
    def test_restoration_job_ordering(self, test_db_session):
        ...
        jobs_data = [
            {
                "user_id": "test_user",
                "original_image_url": f"https://test.example.com/image{i}.jpg",
            }
            for i in range(3)
        ]
    ...
    def test_restoration_job_string_representations(self, test_db_session):
        ...
        # Arrange
        long_url = (
            "https://very-long-domain-name.cloudfront.net/" + "a" * 200 + "/image.jpg"
        )
        long_error = (
            "A very long error message that might occur during processing: " + "x" * 500
        )
