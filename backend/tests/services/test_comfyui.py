...
class TestComfyUIService:
    ...
    def test_download_image_no_subfolder(self, comfyui_service):
        ...
        with patch('requests.get') as mock_get:
            ...
            # Assert
            expected_url = f"{COMFYUI_TEST_URL}/view?filename=result.jpg&subfolder=&type=output"
            mock_get.assert_called_once_with(expected_url)
        ...
        with patch('builtins.open', mock_open(read_data=json.dumps(mock_workflow))):
            with patch.object(comfyui_service, 'upload_image') as mock_upload:
                with patch.object(comfyui_service, 'queue_prompt') as mock_queue:
                    with patch.object(comfyui_service, 'wait_for_completion') as mock_wait:
                        with patch.object(comfyui_service, 'download_image') as mock_download:
                            # Setup mocks
                            ...
                            # Act
                            result = comfyui_service.restore_image(image_data, filename, denoise, megapixels)
                            ...
                            assert queue_call_args["93"]["inputs"]["megapixels"] == megapixels
                            assert queue_call_args["3"]["inputs"]["denoise"] == denoise
                            assert "seed" in queue_call_args["3"]["inputs"]  # Random seed should be set
            ...
            with patch.object(comfyui_service, 'upload_image') as mock_upload:
                with patch.object(comfyui_service, 'queue_prompt') as mock_queue:
                    with patch.object(comfyui_service, 'wait_for_completion') as mock_wait:
                        ...
                        mock_upload.return_value = filename
                        mock_queue.return_value = {"prompt_id": "test_123"}
                        mock_wait.return_value = {"outputs": {"60": {"images": []}}}  # No images

                        # Act & Assert
                        with pytest.raises(ValueError, match="No output images found in result"):
    ...
    def test_restore_image_random_seed_generation(self, comfyui_service, mock_workflow):
        ...
        with patch('builtins.open', mock_open(read_data=json.dumps(mock_workflow))):
            with patch.object(comfyui_service, 'upload_image', return_value=filename):
                with patch.object(comfyui_service, 'queue_prompt', side_effect=capture_seed):
                    with patch.object(comfyui_service, 'wait_for_completion') as mock_wait:
                        with patch.object(comfyui_service, 'download_image') as mock_download:
                            mock_wait.return_value = {"outputs": {"60": {"images": [{"filename": "result.jpg"}]}}}
                            mock_download.return_value = b"processed image data"
    ...
    def test_comfyui_ping_real_instance(self, comfyui_service):
        ...
        try:
            ...
            assert response.status_code == 200, f"ComfyUI not accessible at {comfyui_service.base_url}"
    ...
    def test_comfyui_real_restoration(self, comfyui_service):
        ...
        try:
            ...
            response = requests.get(f"{comfyui_service.base_url}/system_stats", timeout=5)
            if response.status_code != 200:
                pytest.skip(f"ComfyUI instance not available at {comfyui_service.base_url}")
        except requests.exceptions.RequestException:
            pytest.skip(f"ComfyUI instance not accessible at {comfyui_service.base_url}")
        ...
        try:
            ...
            assert result_image.format in ['JPEG', 'PNG'], "Result should be a valid image"
            print(f"✅ Real restoration successful! Result image: {result_image.size} {result_image.format}")
