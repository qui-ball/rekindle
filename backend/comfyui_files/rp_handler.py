...
def handler(job):
    ...
    # Check if caller wants output data included (for datacenters without S3 API)
    include_output_data = job_input.get("include_output_data", False)

    if include_output_data:
        # Return file paths with base64-encoded data
        files_with_data = read_output_files(file_paths, include_data=True)
        return {
            "prompt_id": pid,
            "files": file_paths,  # Keep for backwards compatibility
            "files_with_data": files_with_data,
            "file_count": len(file_paths),
        }
    else:
        # Return just file paths (original behavior)
        return {"prompt_id": pid, "files": file_paths, "file_count": len(file_paths)}
